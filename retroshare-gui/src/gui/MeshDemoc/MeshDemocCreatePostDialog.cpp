...
#include "ui_MeshDemocCreatePostDialog.h"
...
MeshDemocCreatePostDialog::MeshDemocCreatePostDialog(TokenQueue* tokenQ, RsMeshDemoc *posted, const RsGxsGroupId& grpId, QWidget *parent):
	QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
	mTokenQueue(tokenQ), mMeshDemoc(posted), mGrpId(grpId),
	ui(new Ui::MeshDemocCreatePostDialog)
{
	ui->setupUi(this);
...
MeshDemocCreatePostDialog::~MeshDemocCreatePostDialog()
{
	delete ui;
}

void MeshDemocCreatePostDialog::createPost()
{
	RsGxsId authorId;
	...
	uint32_t token;
	mMeshDemoc->createPost(token, post);
